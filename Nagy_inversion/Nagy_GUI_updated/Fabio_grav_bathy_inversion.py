...
from scripts.lsq_linear import lsq_linear
...
import gui_files.sample_geometry    #<---- add .py geometry file


warnings.filterwarnings("ignore")

class DesignerMainWindow(QtWidgets.QMainWindow, gui_files.sample_geometry.Ui_MainWindow):        #<---- add .Ui geometry file
...
#        self.actionHelp.triggered.connect(self.help)
    ...
    def gridplot_input_data_fun(self):
        ...
        #converts input resolutions to floats and assigns variables
...
#        DENS_CORRG=DENS_CORRG_lsq_linear.x
    ...
    def Save_output_file_fun(self):
        ...
        FILEOUT=str(FILEOUT[:-18]) # 'All Files (*)') is being included in file name, not working
        if FILEOUT:
            f=open(FILEOUT, 'w')
