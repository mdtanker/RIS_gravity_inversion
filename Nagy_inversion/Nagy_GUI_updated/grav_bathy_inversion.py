...
#from scripts import gravbox
#from scripts import grav_column_der 
...
from gui_files import grav_bathy_inversion_GUI_geometry    #<---- add .py geometry file
...
# use Object Oriented Programming because GUI's are interactive, so MainWindow will need to change based on user input
    ...
    # define all the function for the above actions     
        ...
        try:
            ...
            if FILEGRAVITY:
                INPUT_GRAVITY=pd.read_csv(FILEGRAVITY, header=0, index_col=None, sep=',')
            ...
            if FILEICE:
                INPUT_ICE=pd.read_csv(FILEICE, header=0, index_col=None, sep=',')
            ...
            if FILEBATHYMETRY:
                INPUT_BATHYMETRY=pd.read_csv(FILEBATHYMETRY, header=0, index_col=None, sep=',')
            ...
            if FILEBASEMENT:
                INPUT_BASEMENT=pd.read_csv(FILEBASEMENT, header=0, index_col=None, sep=',')
            ...
            if FILEMOHO:
                INPUT_MOHO=pd.read_csv(FILEMOHO, header=0, index_col=None, sep=',')
    ...
    def Run_Gravity_Inversion_Func(self):
        ...
        while DCHISQ > 1+Tolerance_Least_Squares: # while DCHISQ is greater than 1 + least squares tolerance (0.02)
            ...
            for i in range(0,NG):  
                ...
                self.progressBar.setProperty("value", 100*(i+1)/NG)
                FA_Forward_Bath[i]=np.sum(gravbox(XG[i],YG[i],ZG[i],     XBath-0.5*RESBATH,XBath+0.5*RESBATH,YBath-0.5*RESBATH,YBath+0.5*RESBATH,    Z_Bath_Base_Moho[0:NBath],    Z2Bath,    RHO_Bath_Base_Moho[0:NBath] - RHOW_Bath)) 
                FA_Forward_Base[i]=np.sum(gravbox(XG[i],YG[i],ZG[i],     XBase-0.5*RESBASE,XBase+0.5*RESBASE,YBase-0.5*RESBASE,YBase+0.5*RESBASE,     Z_Bath_Base_Moho[NBath:NBath+NBase],    Z2Base,     RHO_Bath_Base_Moho[NBath:NBath+NBase] - RHOBath_Base))
                FA_Forward_Moho[i]=np.sum(gravbox(XG[i],YG[i],ZG[i],   XM-0.5*RESM,XM+0.5*RESM,YM-0.5*RESM,YM+0.5*RESM,    Z_Bath_Base_Moho[NBath+NBase:NBath+NBase+NM],    Z2M,     RHO_Bath_Base_Moho[NBath+NBase:NBath+NBase+NM] - RHOM-RHOBase_M))
