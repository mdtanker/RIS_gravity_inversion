...
def forward_grav_plotting(
    ...
    ----------
    df_forward : pd.DataFrame
        Dataframe output from func: forward_gravity_layers()
    layers : dict
        Nested dict; where each layer is a dict with keys:
            'spacing': int, float; grid spacing
            'fname': str; grid file name
    ...
    grav_spacing : float, optional
        spacing of the gravity data for create plots, by default None
    ...
    Returns
    -------
    tuple(layers, forward_grav_total)
        Returns updated layers dict, with gridded forward gravity of each layer, and a
        grid of the total forward gravity.
    """

    # if inversion region not supplied, extract from dataframe
    if inversion_region is None:
        inversion_region = vd.get_region((df_forward.x, df_forward.y))
    ...
    for k, v in layers.items():
        v["forward_grav_grid"] = pygmt.xyz2grd(
            data=df_forward[["x", "y", f"{k}_forward_grav"]],
            region=inversion_region,
            spacing=grav_spacing,
            registration="p",
        )

    forward_grav_total = pygmt.xyz2grd(
        data=df_forward[["x", "y", "forward_total"]],
        region=inversion_region,
        spacing=grav_spacing,
        registration="p",
    )
    ...
    if exclude_layers is not None:
        ...
        exclude_layers_list = [f"{i}_forward_grav" for i in exclude_layers]
        df = df_forward.drop(exclude_layers_list, axis=1)
    else:
        df = df_forward.copy()
        include_forward_layers = pd.Series([k for k, v in layers.items()])

    # setup subplot figure
    fig, ax = plt.subplots(ncols=len(df.columns[4:]), nrows=1, figsize=(20, 20))

    forward_grav_total.plot(
        ax=ax[0],
        x="x",
        y="y",
        robust=True,
    ...
    for i, j in enumerate(include_forward_layers):
        # plot each grid
        layers[j]["forward_grav_grid"].plot(
            ax=ax[i + 1],
            x="x",
            y="y",
            robust=True,
    ...
    return layers, forward_grav_total
...
def anomalies_plotting(
    ...
    **kwargs,
    ...
    ----------
    df_anomalies : pd.DataFrame
        Should be dataframe output from func: anomalies()
    inversion_region : list, optional
        GMT format region for the inverion, by default is extent of gravity data
    grav_spacing : float, optional
        spacing of the gravity data for create plots, by default None
    plot_constraints : bool, optional
        Choose whether to plot constraint points on the maps, by default False
    plot_dists : bool, optional
        Choose whether to plot the resulting distributions, by default False
    plot_power_spectrums : bool, optional
        Choose to plot radially average power spectrum of layers, by default False
    ...
    # if inversion region not supplied, extract from dataframe
    if inversion_region is None:
        inversion_region = vd.get_region((df_anomalies.x, df_anomalies.y))
    ...
    if (plot_constraints is True) and (constraints is None):
        raise ValueError(
            "If plot_constraints is True, must provide dataframe with 'constraints'"
        )

    # get columns to include in gridding
    cols_to_grid = ["Gobs", "grav_corrected", "forward_total", "misfit", "reg", "res"]
    ...
    # plot the grids
    fig, ax = plt.subplots(ncols=len(anom_grids), nrows=1, figsize=(20, 20))

    for i, j in enumerate(anom_grids):
        j.plot(
            ax=ax[i],
            x="x",
            y="y",
...
def plot_inversion_results(
    ...
    **kwargs,
    ...
    plot_type: {'xarray', 'pygmt'},
            ...
            'xarray'
    plot_region : Union[str, np.ndarray], optional
        GMT-format region to plot for both 2d and 3d plots, by default is inversion
        region
    ...
    if plot_constraints is True and constraints is None:
        raise ValueError(
            "If plot_constraints is True, must provide parameter 'constraints'"
        )
    ...
    if plot_type == "pygmt":
        ...
        maxabs = vd.maxabs(grav) * 0.95
        grav_lims = (-maxabs, maxabs)

        for ITER in iterations:
            initial_RMS = round(
                np.sqrt(
                    (input_grav[f"iter_{ITER}_initial_misfit"] ** 2).mean(skipna=True)
                ),
                2,
            )
            # final_RMS = round(
            #     np.sqrt(
            #         (input_grav[f"iter_{ITER}_final_misfit"] ** 2)
            #         .mean(skipna=True)
            #         .item()
            #     ),
            #     2,
            ...
            correction = round(
                np.sqrt(
                    (iter_corrections[f"iter_{ITER}_correction"] ** 2)
                    .mean(skipna=True)
                    .item()
                ),
                2,
            ...
            if ITER == 1:
                ...
                misfit_lims = (-vd.maxabs(grid1) * 0.85, vd.maxabs(grid1) * 0.85)
                corr_lims = (-max_layer_change_per_iter, max_layer_change_per_iter)
                ...
                # percent = 2
                # topo_lims = (
                #     np.nanquantile(masked, q=percent / 100),
            ...
            grid2.plot(
                ...
                x="x",
                y="y",
                robust=True,  # vmin=topo_lims[0], vmax=topo_lims[1],
                cmap="gist_earth",
                add_labels=False,
                cbar_kwargs={"orientation": "horizontal", "anchor": (1, 1.8)},
            )
        ...
        (grav - initial_forward_total).plot(
            ax=ax,
            x="x",
            y="y",
            robust=True,
